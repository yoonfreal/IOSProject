...
private enum HazardKind: String, CaseIterable, Identifiable {
    case books, lamp, clock
    ...
    var hint: String {
        switch self {
        case .books: return "Take the books down from the top shelf"
        case .lamp: return "Move the lamp off the table"
        case .clock: return "Take the clock down from the wall"
        }
    }
}

private struct Hazard: Identifiable {
    let id = UUID()
    let kind: HazardKind
    var start: CGPoint
    var target: CGPoint
...
struct PrepRoomView: View {
    ...
    private let snapThreshold: CGFloat = 70
            ...
            VStack(spacing: DesignTokens.Spacing.md) {
                ...
                VStack(spacing: DesignTokens.Spacing.xs) {
                    ...
                    Text(currentHint)
                        ...
                        .animation(DesignTokens.Motion.easeGentle, value: currentHint)
                    ProgressPill(placed: placedCount, total: hazards.count)
                        .padding(.top, 2)
                }
                ...
                GeometryReader { proxy in
                    ...
                    ZStack {
                        ...
                        ForEach(hazards) { hazard in
                            SafeZoneMarker(placed: hazard.placed)
                                .frame(width: hazard.size.width + 14, height: hazard.size.height + 14)
                                .position(hazard.target)
                        }

                        ForEach($hazards) { $hazard in
                            let isNext = hazard.id == nextHazardID
                            HazardShape(kind: hazard.kind, placed: hazard.placed)
                                .frame(width: hazard.size.width, height: hazard.size.height)
                                .background(
                                    Circle()
                                        .fill(DesignTokens.Palette.calmAccent.opacity(isNext ? 0.28 : 0))
                                        .frame(
                                            width: max(hazard.size.width, hazard.size.height) * 1.9,
                                            height: max(hazard.size.width, hazard.size.height) * 1.9
                                        )
                                        .scaleEffect(isNext && hintPulse ? 1.15 : 0.95)
                                        .opacity(isNext ? (hintPulse ? 0.9 : 0.4) : 0)
                                        .blur(radius: 6)
                                )
                                .position(hazard.start)
                                .offset(hazard.offset)
                                .zIndex(draggingID == hazard.id ? 10 : 3)
                                .gesture(dragGesture(for: $hazard))
                                .accessibilityLabel("\(hazard.kind.label). \(hazard.placed ? "Placed safely." : hazard.kind.hint).")
                        }
                    }
                    .onAppear {
                        if hazards.isEmpty {
                            hazards = makeInitialHazards(in: proxy.size)
                        }
                    }
                }
                ...
                Button("Continue", action: onContinue)
                    ...
                    .animation(DesignTokens.Motion.easeGentle, value: allPlaced)
                    .accessibilityLabel(allPlaced ? "Continue to the next screen" : "Place all hazards in safe zones to continue")
    ...
    private var allPlaced: Bool {
        !hazards.isEmpty && hazards.allSatisfy { $0.placed }
    }

    private var placedCount: Int {
        hazards.filter { $0.placed }.count
    }

    private var nextHazard: Hazard? {
        hazards.first { !$0.placed }
    }

    private var nextHazardID: UUID? {
        nextHazard?.id
    }

    private var currentHint: String {
        if hazards.isEmpty {
            return "Drag each hazard down to a safe spot on the floor."
        }
        if let next = nextHazard {
            return "\(next.kind.hint) — drag it to the highlighted spot."
        }
        return "Nice work — every hazard is safely on the floor."
    }

    private func makeInitialHazards(in size: CGSize) -> [Hazard] {
        let w = size.width
        let h = size.height
        return [
            Hazard(
                kind: .books,
                start: CGPoint(x: w * 0.82, y: h * 0.34),
                target: CGPoint(x: w * 0.78, y: h * 0.91),
                size: CGSize(width: w * 0.16, height: h * 0.06)
            ),
            Hazard(
                kind: .lamp,
                start: CGPoint(x: w * 0.42, y: h * 0.62),
                target: CGPoint(x: w * 0.28, y: h * 0.90),
                size: CGSize(width: w * 0.09, height: h * 0.13)
            ),
            Hazard(
                kind: .clock,
                start: CGPoint(x: w * 0.56, y: h * 0.12),
                target: CGPoint(x: w * 0.52, y: h * 0.91),
                size: CGSize(width: w * 0.11, height: h * 0.11)
            )
        ]
    }

    private func dragGesture(for hazard: Binding<Hazard>) -> some Gesture {
        DragGesture()
            .onChanged { value in
                guard !hazard.wrappedValue.placed else { return }
                draggingID = hazard.wrappedValue.id
                hazard.wrappedValue.offset = value.translation
            }
            .onEnded { value in
                guard !hazard.wrappedValue.placed else { return }
                draggingID = nil
                let currentPos = CGPoint(
                    x: hazard.wrappedValue.start.x + value.translation.width,
                    y: hazard.wrappedValue.start.y + value.translation.height
                )
                let target = hazard.wrappedValue.target
                let distance = hypot(currentPos.x - target.x, currentPos.y - target.y)

                if distance < snapThreshold {
                    withAnimation(DesignTokens.Motion.spring) {
                        hazard.wrappedValue.offset = CGSize(
                            width: target.x - hazard.wrappedValue.start.x,
                            height: target.y - hazard.wrappedValue.start.y
                        )
                        hazard.wrappedValue.placed = true
                    }
                } else {
                    withAnimation(DesignTokens.Motion.spring) {
                        hazard.wrappedValue.offset = .zero
                    }
                }
            }
    }
...
private struct ProgressPill: View {
    ...
    var body: some View {
        HStack(spacing: DesignTokens.Spacing.xs) {
            Image(systemName: placed == total && total > 0 ? "checkmark.circle.fill" : "hand.draw")
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(placed == total && total > 0 ? DesignTokens.Palette.successAccent : DesignTokens.Palette.calmAccent)
            Text("\(placed) of \(total) placed")
        ...
        .animation(DesignTokens.Motion.easeGentle, value: placed)
        .accessibilityLabel("\(placed) of \(total) hazards placed")
    }
}

private struct SafeZoneMarker: View {
    var placed: Bool

    var body: some View {
        RoundedRectangle(cornerRadius: DesignTokens.Radius.sm)
            .fill((placed ? DesignTokens.Palette.successAccent : DesignTokens.Palette.calmAccent).opacity(0.10))
            .overlay(
                RoundedRectangle(cornerRadius: DesignTokens.Radius.sm)
                    .strokeBorder(
                        (placed ? DesignTokens.Palette.successAccent : DesignTokens.Palette.calmAccent).opacity(0.55),
                        style: StrokeStyle(lineWidth: 1.5, dash: placed ? [] : [5, 4])
                    )
            )
            .overlay(
                Image(systemName: placed ? "checkmark" : "arrow.down")
                    .font(.system(size: 13, weight: .bold))
                    .foregroundStyle(placed ? DesignTokens.Palette.successAccent : DesignTokens.Palette.calmAccent.opacity(0.75))
            )
            .animation(DesignTokens.Motion.easeGentle, value: placed)
    }
}

private struct HazardShape: View {
    var kind: HazardKind
    var placed: Bool
    ...
    var body: some View {
        Group {
            switch kind {
            case .books: BooksStackShape()
            case .lamp: LampShape()
            case .clock: ClockShape()
            }
        }
        .shadow(color: DesignTokens.Palette.softShadow, radius: placed ? 3 : 7, y: placed ? 2 : 4)
    }
}
